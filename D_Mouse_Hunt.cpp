...
ll ans = 0;
vl b;
ll dfs(ll i, vl& nxt, vl& vis, vl& val, vl& cost){
    vis[i] = 1;
    b.pus(i);
    ll nei = nxt[i];
    if(!vis[nei]) val[i] = dfs(nei,nxt,vis,val,cost);
    else if (vis[nei] == 2) val[i] = val[nei];
    else {
        val[i] = cost[nei];
        for(auto it = b.rbegin() ; *it != nei; it++){
            val[i] = min(val[i] , cost[*it]);
        }
        ans += val[i];
    }
    b.pop_back();
