...
//-------------------------------------------------------------------------------------------------------------------------------------
// i know a can only be l or r
// so use dp to get the maximum value possible
ll dp[100004][2];
vl l,r;
void dfs(ll i, vvl& al, ll p = -1){
    for(auto& ch : al[i]){
        if(ch != p){
            ...
            dp[i][0] += max( dp[ch][0] + abs(l[i]-l[ch]) , dp[ch][1] + abs(l[i]-r[ch]) );
            dp[i][1] += max( dp[ch][0] + abs(r[i]-l[ch]) , dp[ch][1] + abs(r[i]-r[ch]) );
        }
    }
}
