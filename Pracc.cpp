...
int f(int n, int k, vector<int>& pr, vector<int>& b){
    vector<vector<int>> nums;
    for(int i = 0; i < n; i++){
        nums.push_back({b[i],pr[i]});
    }
    ...
    for(int cnt = 0; cnt < k; cnt++){
        sum += nums[n-1-cnt][1];
        int val = nums[n-1-cnt][0]*sum;
        ans = max(ans,val);
        pq.push(nums[n-1-cnt][1]);
        if(pq.size() > k-1) {
            int el = pq.top();
            sum -= el;
            pq.pop();
        }
        // cout<<n-1-cnt<<" : "<<val<<" & ans is "<<ans;nl
    }
    for(int i = n-k-1; i >= 0; i--){
        int val = nums[i][0]*(sum+nums[i][1]);
        ans = max(ans,val);
        if(nums[i][1] > pq.top()){
            int el = pq.top();
            pq.pop();
