...
//-------------------------------------------------------------------------------------------------------------------------------------
double d(pll a, pll b){
    ll dd = power(a.ff - b.ff, 2LL) + power(a.ss - b.ss, 2LL);
    return sqrtl(dd);
}
void solve(){
    ...
    double bo = d(b, o);
    double r = d(a, b) / 2.0;
    // ans is min of either r or in same circle
    double same = min(max(ao, ap), max(bo, bp));
    r = max({r,min(ao,bo),min(ap,bp)});
    cout<<min(same,r);nl
}
