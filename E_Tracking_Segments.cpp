...
bool is(ll mid, vpl& seg, vl& qr, ll n){
    vl arr(n);
    For(mid+1) arr[qr[i]] = 1;
    vl pre(n);
    For(n) (i > 0) ? pre[i] = arr[i]+pre[i-1] : pre[i] = arr[i];
    ll m = seg.size();
    bool flag = 0;
    For(m){
        ll l = seg[i].ff;
        ll r = seg[i].ss;
        ll num = (l > 0) ? pre[r]-pre[l-1] : pre[r];
        if(2*num > r-l+1){
            flag = 1;
            break;
        }
