...
void solve(){
    ...
    if(k == 2){
        For(n){
            if(a[i]%2 == 0) ans = min(ans,0);
            else ans = min(ans,1);
        }
    }
    else if(k == 3){
        For(n){
            if(a[i]%3 == 0) ans = min(ans,0);
            else if(a[i]%3 == 1)ans = min(ans,2);
            else ans = min(ans,1);
        }
    }
    else if(k == 5){
        For(n){
            if(a[i]%5 == 0) ans = min(ans,0);
            else if(a[i]%5 == 1)ans = min(ans,4);
            else if(a[i]%5 == 2)ans = min(ans,3);
            else if(a[i]%5 == 3)ans = min(ans,2);
            else ans = min(ans,1);
        }
    }
    else{
        int odd = 0, even = 0;
        For(n){
            if(a[i]%4 == 0) ans = min(ans,0);
            else if(a[i]%4 == 1) ans = min(ans,3);
            else if(a[i]%4 == 2) ans = min(ans,2);
            else ans = min(ans,1);
            if(a[i]&1) odd++;
            else even++;
        }
        ...
        if(odd > 0 and even > 0) ans = min(ans,1);
    }
    cout<<ans;nl
}
