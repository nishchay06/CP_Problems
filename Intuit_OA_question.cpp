...
//-------------------------------------------------------------------------------------------------------------------------------------
/*
you are given a string s consisting of n alpha numerc characters you need to find the longest substring of s that is a valid cipher

given k3Cb the substrings that are valid cipher are C and Cb.Note that KCb is not a substrint and k3B is not a valid cipher

in this case your function should return 2 as the longest substring in Cb. alternatively given k3uu your function should return -1 since their is not substring that satisfies the restriction on a format of a valid cipher
...
void solve(){
    ...
    for(auto& ch : s){
        if(isdigit(ch)){
            if(good) ans = max(ans,cr);
            cr = 0;
            good = 0;
            continue;
        }
        if(ch >= 'A' and ch <= 'Z') good = 1;
        cr++;
    }
    cout<<ans;nl
}
