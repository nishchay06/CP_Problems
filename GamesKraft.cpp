...
//-------------------------------------------------------------------------------------------------------------------------------------
void countBits(ll node, vector<vpl> &adjl, vvl &bitCount, ll par = -1) {
  for (auto &it : adjl[node]) {
    int ch = it.first;
    ll wt = it.second;
    if (ch != par) {
      For(20) bitCount[ch][i] = bitCount[node][i];
      For(20) {
        if (wt & (1 << i)) {
          bitCount[ch][i]++;
        }
      }
      countBits(ch, adjl, bitCount, node);
    }
  }
}
void solve(){
  ...
  cout << n << " " << q << endl;
  For(n-1){
    cout << tree_from[i] << " " << tree_to[i] << " " << tree_weight[i] << endl;
  }
  vector<vpl> adjl(n);
  For(n-1){
    adjl[tree_from[i] - 1].pus({tree_to[i] - 1, tree_weight[i]});
    adjl[tree_to[i] - 1].pus({tree_from[i] - 1, tree_weight[i]});
  }
  vvl numBits(n, vl(20, 0));
  // countBits(0, adjl, numBits);
}
