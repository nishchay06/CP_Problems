...
void solve(){
    ...
    ll add = 0;
    for(auto& [val,f] : mpgood) {
        f += add;
        add = f;
    }
    
    ll prevfreq = 0;
    for(auto& [cst,badfreq] : mpbad) {
        badfreq += prevfreq;
        if(prevfreq <= k) {
            // prevgoodfreq = goodrev at cst-1
            ll prevgoodfreq = 0;
            auto it = mpgood.upper_bound(cst-1);
            it--;
            prevgoodfreq = it->ss;
            ll cr = (cst-1)*(prevfreq+prevgoodfreq);
            ans = max(ans,cr);
        }
        prevfreq = badfreq;
    }
