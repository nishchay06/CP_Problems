...
void binLift(vvl& al, vvl& prts, ll n){
...
void solve(){
    ...
    vvl adjl(numOfNodes);
    for(int i = 2; i < 2+numOfNodes-1; i++){
        ll p;
        cin>>p;
        p--;
        adjl[p].push_back(i-1);
        adjl[i-1].push_back(p);
    }
    prts.resize(log2(numOfNodes)+1,vl(numOfNodes,-1));
    dfs(0,adjl);
    binLift(adjl,prts,numOfNodes);
    For(numOfQueries){
        ll x,k;
        cin >> x >> k;
        ll ans = getKthParent(x-1,k);
        (ans != -1) ? cout<<ans+1 : cout<<ans;nl
    }
}
