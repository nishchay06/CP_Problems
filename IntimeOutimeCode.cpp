...
void getTimes(ll i){
    cout<<inTime[i]<<" "<<outTime[i];nl
}

bool isAncestor(ll i, ll j){
    return (inTime[i] < inTime[j] and outTime[i] > outTime[j]);
}

void solve(){
    // first enter a tree => enter n the number of nodes and n-1 edges;
    ll numOfNodes;
    cin >> numOfNodes;
    // getting edges;
    vvl adjl(numOfNodes);
    ...
    Time = 0;
    inTime.resize(numOfNodes,0);
    outTime.resize(numOfNodes,0);
    dfs(0,adjl);
    For(numOfNodes){
        cout<<i<<": ";
        getTimes(i);
