...
#define Forst(n)     for (ll st = 0; st < n; st++)
...
vl sieve(ll n) {vl a(n+1,1),v; a[0]=a[1]=0; Forl(i,2,n+1) if(a[i]) {v.pus(i); for(ll st=i*i; st <=n; st+=i) a[st]=0;} return a;}
vl divsr(ll n) {vl a(n+1,1); For(n+1) a[i]=i; Forl(i,2,n+1) if(a[i]==i) {for(ll st=2*i; st<=n; st+=i) a[st]=i;} return a;}
...
/* stuff you should look for 
    ...
    * check for time cofreqTablelexity (make sure not to TLE)
    * special case (n = 1)
...
void solve(){
    string s;
    cin >> s;
    // length of the longest substring without repeating charachters
    int maxLen = 0;
    string resStr = "";
    map<char,int> freqTable;
    int n = s.size();
    for(int end = 0, st = 0; end < n; end++){
        freqTable[s[end]]++;
        int curLen = end-st+1;
        if(curLen == freqTable.size()){
            if(curLen > maxLen){
                maxLen = curLen;
                resStr = s.substr(st,curLen);
            }
        }
        while(st <= end and curLen > freqTable.size()){
            freqTable[s[st]]--;
            if(freqTable[s[st]] == 0) freqTable.erase(s[st]);
            st++;
            curLen--;
        }
    }
    cout<<maxLen<<" "<<resStr;nl
}
