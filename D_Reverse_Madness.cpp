...
void solve(){
    ...
    vl qu(q);
    inpt(qu);
    priority_queue<pll> ch;
    For(q){
        ll x = qu[i];
        ll ind = lowB(r,x);
        ll lim = r[ind]+l[ind]-x;
        ll mn = min(lim,x);
        ll mx = max(lim,x);
        if(mn == mx) continue;
        ch.push({mx,mn});
    ...
    while(sz(ch)){
        ...
        ll crmx = ch.top().ff;
        ll num = 0;
        while(sz(ch) and ch.top().ff == crmx and ch.top().ss == crmn){
            num++;
            ch.pop();
        }
        if(num&1){
            swap(s[crmn-1],s[crmx-1]);
            if(crmx-1 > crmn+1) ch.push({crmx-1,crmn+1});
        }
    }
    cout<<s;nl
}
