...
//-------------------------------------------------------------------------------------------------------------------------------------
int dp[300005];
ll dfs(ll cr, vl& up, vl& left, vl& right, str& s){
    if(cr == 0) return 1e9;
    if(left[cr] == 0 and right[cr] == 0) return 0;
    if(dp[cr] != -1) return dp[cr];
    dp[cr] = min(dfs(left[cr],up,left,right,s)+(s[cr-1] != 'L'),dfs(right[cr],up,left,right,s)+(s[cr-1]!='R'));
    // cout<<cr<<" -> "<<dp[cr];nl
    return dp[cr];
}
void solve(){
    ll n; cin >> n;
    vl up(n+1), left(n+1), right(n+1);
    str s; cin >> s;
    For(n){
        ll cr = i+1;
        ll l,r;
