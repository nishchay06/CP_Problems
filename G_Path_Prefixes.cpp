...
void dfs(ll i, ll p, ll sum, vl& pre, vl& res, vector<pair<ll,pair<ll,ll>>> al[]){
    ll ind = upB(pre,sum);
    res[i] = ind-1;
    for(auto& ch : al[i]){
        if(ch.ff != p){
            pre.pus(pre.back()+ch.ss.ss);
            dfs(ch.ff,i,sum+ch.ss.ff,pre,res,al);
            pre.pop_back();
        }
    }
}
void solve(){
    ll n;
    cin>>n;
    vector<pair<ll,pair<ll,ll>>> al[n+1];
    For1(n+1){
        if(i == 1) continue;
        ll p,a,b;
        cin>>p>>a>>b;
    ...
    vl pre = {0};
    vl res(n+1,0);
    dfs(1,-1,0,pre,res,al);
    Forl(i,2,n+1)cout<<res[i]<<" ";
    nl
}
